...
enum DisplayTypes {TEMPPLOT, CONCPLOT, RHO, PRESSUREFOO,
#ifdef REACTIVE
FRACTIONPLOT,
#endif
...
*/
    ...
    } else if (plotVariable == TEMPPLOT) {
      data[ni * j + i] = grid(gi, gj)[TEMP];
    } else if (plotVariable == CONCPLOT) {
      data[ni * j + i] = grid(gi, gj)[CONCPLOT];
    }
  }
}
